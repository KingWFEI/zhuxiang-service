...
public class OpenApiDescriptionConfig {
    ...
    private static final Map<String, String> SCHEMA_DESCRIPTIONS = Map.ofEntries(
            ...
            entry("RecordRequest", "开锁结果上报"),
            entry("RecordResponse", "开锁记录写入结果"),
            entry("AvatarResult", "头像上传结果"),
            entry("ContractPreviewResponse", "租赁合同预览"),
            entry("ContractDetail", "房东工作台合同详情"),
            entry("ContractItem", "房东工作台合同列表项"),
            ...
            entry("LeaseLockPasscodeResponse", "租客期限密码详情"),
            ...
            entry("UnlockDataResponse", "租约门锁权限摘要"),
    ...
    private static final Map<String, String> PROPERTY_DESCRIPTIONS = Map.ofEntries(
            ...
            entry("bluetoothUnlockAvailable", "蓝牙开锁权限当前是否可用"),
            entry("autoUnlockAvailable", "当前租约和门锁是否允许无感开锁"),
            entry("autoUnlockCooldownSeconds", "无感开锁成功后的冷却秒数"),
            entry("autoUnlockMinRssi", "无感开锁稳定窗口平均 RSSI 下限"),
            entry("autoUnlockStableMillis", "无感开锁信号稳定持续毫秒数"),
            entry("autoUnlockGeofenceRadiusMeters", "进入房屋范围并启动蓝牙扫描的半径，单位米"),
            entry("autoUnlockExitRadiusMeters", "重新布防所需的离开半径，单位米"),
            entry("autoUnlockExitDwellSeconds", "重新布防所需的持续离开秒数"),
            entry("autoUnlockScanWindowSeconds", "进入房屋范围后的蓝牙扫描窗口秒数"),
            entry("autoUnlockMinRssiSamples", "达到开锁信号阈值所需的最少稳定采样数"),
            entry("building", "楼栋"),
            ...
            entry("enabled", "是否启用"),
            entry("endDate", "结束日期"),
            entry("endTime", "权限失效时间"),
            entry("expiresIn", "有效期，单位秒"),
            entry("firstUseNotice", "密码首次使用时限提示"),
            ...
            entry("homeBanners", "首页 Banner 广告列表"),
            entry("houseId", "房源 ID"),
            ...
            entry("pageSize", "每页条数"),
            entry("paidAt", "付款完成时间"),
            entry("passcode", "租约期限密码，仅通过专用接口返回"),
            entry("passcodeAvailable", "期限密码当前是否可用"),
            entry("passcodeEndTime", "期限密码失效时间"),
            entry("passcodeStartTime", "期限密码生效时间"),
            entry("passcodeStatus", "期限密码权限状态"),
            entry("passcodeType", "密码类型，本期为 PERIOD"),
            ...
            entry("rentMode", "出租方式：WHOLE_RENT整租，SHARED_RENT合租"),
            entry("rentType", "租赁类型：LONG_RENT长租，SHORT_RENT短租，HOMESTAY民宿"),
            ...
            entry("tenantId", "租客用户 ID"),
            entry("tenantIdCard", "租客身份证号"),
            entry("tenantName", "租客姓名"),
            entry("tenantPhone", "租客联系电话"),
            entry("tenantSigned", "租客是否已完成签署"),
            entry("title", "标题"),
            entry("total", "总记录数或合计数量"),
            entry("totalAmount", "订单总金额，单位元"),
            entry("ttlockKeyId", "TTLock 租客电子钥匙 ID"),
            entry("ttlockLockId", "TTLock 平台门锁 ID"),
            entry("triggerType", "开锁触发方式：AUTO_NEARBY 或 MANUAL_BLUETOOTH"),
            entry("stableMillis", "信号稳定毫秒数，仅 AUTO_NEARBY 必填"),
            entry("failureReason", "脱敏后的开锁失败错误码"),
            entry("deviceInfo", "客户端设备与系统摘要"),
            entry("appVersion", "客户端版本"),
            entry("type", "条目类型"),
            entry("unit", "单元"),
